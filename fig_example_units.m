...
% cc = cc + 1;
unitId = unitList(cc);
[stimconds, robs, B, opts] = bin_ssunit(D, unitId, 'win', [-.2 .2], 'plot', false, 'binsize', 10e-3);

kern = mean(filtfilt(boxcar(5)/5, 1, robs'),2);
kern = kern - mean(kern(opts.lags < 0));
kern = kern - min(kern);
kern = kern .* (opts.lags > 0 & opts.lags <  opts.lags(end)-.2)';
kern = kern / sum(kern);
clf
plot(kern)

R = robs*kern/opts.binsize;
...
subplot(2,3,1)
plot(opts.lags, psth'/bs); hold on
plot(opts.lags, 5*kern/opts.binsize, 'k')
title(numel(stimDir))
...
subplot(2,3,5)
plot(directions, (x'*R)./sum(x)'); hold on
plot(directions, (x'*sum(robs(:,opts.lags > 0 & opts.lags < 1),2))./sum(x)')
