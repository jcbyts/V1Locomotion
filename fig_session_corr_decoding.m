...
%         pval = cellfun(@(x) x.pval(ipc), RunCorr.(subject)(sessix));
        ...
        if p < 0.05
            text(median(rho), 1.05*max(ylim), '*', 'FontSize', 12)
        end
        plot.offsetAxes(gca)
        ylabel('# sessions')
...
%         title(['$\rho:$ ' num2str(RunCorr.(subject){isess}.rho(1), 3), ',   $r^2:$' num2str(RunCorr.(subject){isess}.decoding_r2, 3)], 'interpreter', 'latex')
        ...
        axis off
        hold on
        plot([nt nt]-20, size(robs,2)-[0 10], 'k', 'Linewidth', 1)
        text(nt, size(robs,2), sprintf('%d neurons', 10), 'FontSize',6, 'FontName', 'Helvetica', 'Rotation', 90);
        colormap(cmap)
...
%         plot(rpc(:,1)*beta(1)^2 + beta(2), 'Color', cmap(6,:));
        ...
        plot([1 1]*(nt-20), 10+[0 mxsp], 'k', 'Linewidth', 1)
