...
for isubj = 1:nsubjs
    ...
    if exclude_calcarine
        ...
        if isfield(Stat.(subject), 'sessid')
            good = intersect(good, find(~ismember(Stat.(subject).sessid, [13, 15])));
        end
    else
...
%     export_fig(gcf, fullfile(figdir, sprintf('osi_dist_%s.pdf', subject)), '-pdf', '-transparent')

%     figure(isubj*20+2); clf
%     set(gcf, 'Visible', 'off')
%     figure('Visible', 'on', 'Position', [0 0 300*[3 1]])
...
unit_groups = {'all', 'tuned', 'vis', 'suppressed', 'foveal', 'peripheral', 'npx_calc', 'npx_fov'};
...
for igroup = 1:numel(unit_groups)
    ...
    for isubj = 1:nsubjs
        ...
        switch plot_index
            ...
            case 'npx_calc'
                if isfield(Stat.(subject), 'sessid')
                    good = good & pvis & ismember(Stat.(subject).sessid, [13, 15]);
                else
                    good = good & pvis; % for mouse use vis condition
                end
            case 'npx_fov'
                if isfield(Stat.(subject), 'sessid')
                    good = good & pvis & ismember(Stat.(subject).sessid, [14, 16]);
                else
                    good = good & pvis; % for mouse use vis condition
                end
...
%         title('Preferred Stim firing rate')
        ...
        % STIM-DRIVEN
        if strcmp(yscale, 'log')
%             figure(isubj*10+2, 'Visible', 'off'); clf
            figure('Visible', 'off')
